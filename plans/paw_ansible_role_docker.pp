...
#
# This plan parses Ansible inventory, filters targets, and executes the paw_ansible_role_docker task
...
#   puppet plan run paw_ansible_role_docker inventory_source=/path/to/inventory
#
# @example Execute on hosts from API
#   puppet plan run paw_ansible_role_docker inventory_source=https://cmdb.example.com/inventory
#
# @example Execute on specific hosts
#   puppet plan run paw_ansible_role_docker inventory_source=/path/to/inventory limit='webservers'
#
# @example Execute with custom concurrency
#   puppet plan run paw_ansible_role_docker inventory_source=/path/to/inventory parallel=true concurrency=5
#
# @example Execute in check mode with verbose output
#   puppet plan run paw_ansible_role_docker inventory_source=/path/to/inventory par_check_mode=true par_verbose=true
#
plan paw_ansible_role_docker (
  ...
  $results = if $parallel {
    ...
    $targets.slice($concurrency).map |$batch| {
      parallelize($batch) |$target| {
        ...
        $result = run_task('paw_ansible_role_docker', $target['host'], $task_params)
        
        # Return structured result
        $task_result = {
          'host'      => $target['host'],
          'task'      => 'paw_ansible_role_docker',
